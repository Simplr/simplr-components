...
export default class SimplrDataTable extends LitElement {
  ...
  isTableButtonClick(e) {
    // If icon on table was clicked, one of the first 10 elements in the path
    // should contain tableButton in their dataset
    return e.path.splice(0, 10).some(elem => {
      if (!elem.dataset) return false;
      return Object.prototype.hasOwnProperty.call(elem.dataset, 'tableButton');
    });
  }

  handleButtonClickEvent(e, onClick) {
    const rowId = this.findRowId(e);
    onClick.call(
      this,
      this.tableData.find(td => td.id.toString() === rowId),
    );
  }
          ...
          ? html`
              ${dataCol.type === 'button'
                ? html`
                    <simplr-info-button
                      icon="${icon}"
                      content=${dataCol.content}
                      color=${this.getTableCellContentStyles(dataCol)}
                      @click="${e => this.handleButtonClickEvent(e, dataCol.onClick)}"
                      data-table-button
                    ></simplr-info-button>
                  `
                : html`
                    <simplr-icon
                      icon="${icon}"
                      @click="${dataCol.onClick
                        ? e => this.handleButtonClickEvent(e, dataCol.onClick)
                        : null}"
                      data-table-button
                    ></simplr-icon>
