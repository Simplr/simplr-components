...
export default class SimplrDateInput extends SimplrInputBase {
  ...
  _findMwcInput(path) {
    ...
    for (let i = 0; i < path.length; i += 1) {
      if (path[i].nodeName === 'MWC-TEXTFIELD') {
        mwcInput = path[i];
        break;
      }
    }
  ...
  // When using a altinput, a new input field is created,
  // breaking the mwc-textfield ripple functionality,
  // so we handle the focus programmatically
  _handleAltInputFocus(e) {
    if (!this.altInput) return;
    const path = e.path || (e.composedPath && e.composedPath());
    if (!path || typeof path === 'undefined') {
      return;
    }
    const mwcInput = this._findMwcInput(path);
    if (mwcInput) {
      mwcInput.focus();
    }
  }
  ...
  // When using a altinput, a new input field is created,
  // breaking the mwc-textfield ripple functionality,
  // so we handle the blur programmatically
  _handleAltInputBlur(e) {
    if (!this.altInput) return;
    const mwcInput = this._findMwcInput(e.path);
    if (mwcInput) {
      mwcInput.blur();
    }
  }
  ...
  render() {
    return html`
      <lit-flatpickr
        ...
        ?wrap=${this.wrap}
        ><simplr-text-input
          @focus=${e => this._handleAltInputFocus(e)}
          @blur=${e => this._handleAltInputBlur(e)}
          label="${this.label}"
