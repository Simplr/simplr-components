...
export default class SimplrSelect extends LitElement {
  ...
  updateOptionFilter(e) {
    if (e.key === 'Enter') {
      // Select on enter
      const bestMatch = this.shadowRoot.querySelector('option');
      if (bestMatch) {
        bestMatch.click();
      }
      return;
  ...
  createSearchField() {
    return html`
      <div class="search-field${this.isOpen ? 'search-field--open' : ''}">
        <input type="text" id="search-input-field" @keyup=${this.updateOptionFilter} />
      </div>
    `;
  }
  ...
  createSingleSelectElement() {
    return html`
      ...
      >
        <option @click=${() => this.handleSingleSelect()}></option>
